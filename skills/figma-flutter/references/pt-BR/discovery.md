...
- **`get_design_context` pede a skill do Figma carregada antes.** Claude Code: leia o recurso uma vez com `ReadMcpResourceTool(server: "figma", uri: "skill://figma/figma-design-to-code/SKILL.md")` (ou `/figma-design-to-code` se o plugin estiver instalado) e passe `skillNames: "resource:figma-design-to-code"`. Agentes sem leitor de recursos (Codex): instale a skill curada `figma` ou apenas chame a ferramenta — `skillNames` é parâmetro de log (veja `agents.md`). Passe também `clientLanguages: "dart", clientFrameworks: "flutter"` e `excludeScreenshot: true` quando o PNG já estiver salvo — a resposta fica menor.
