...
- **`get_design_context` requires the Figma skill loaded first.** Read the resource once with `ReadMcpResourceTool(server: "figma", uri: "skill://figma/figma-design-to-code/SKILL.md")` (or `/figma-design-to-code` if the plugin is installed) and pass `skillNames: "resource:figma-design-to-code"`. Also pass `clientLanguages: "dart", clientFrameworks: "flutter"` and `excludeScreenshot: true` when you already saved the PNG — the response is smaller.
